...
class CookiesSession:
    ...
    def desde_env(cls) -> "CookiesSession":
        ...
        Si phpsessid no existe, intenta login.
        """
        load_dotenv()
        phpsessid = os.getenv("SCREENERMATIC_PHPSESSID", "")
        usuario = os.getenv("SCREENERMATIC_USER", "")
        password = os.getenv("SCREENERMATIC_PASS", "")

        if not phpsessid and usuario and password:
            try:
                s = requests.Session()
                s.headers.update(HEADERS_NAVEGADOR_CHROME)
                s.post("https://www.screenermatic.com/login.php", data={"email": usuario, "password": password}, timeout=15)
                phpsessid = s.cookies.get("PHPSESSID", "")
                logger.info("Login automático exitoso a Screenermatic.")
            except Exception as e:
                logger.error(f"Error en login automático: {e}")

        if not phpsessid:
            raise ValueError(
                "SCREENERMATIC_PHPSESSID no encontrado en .env y login falló.\n"
...
class CacheScreenermatic:
    ...
    def get_bonos(self, forzar_actualizacion: bool = False) -> pd.DataFrame:
        ...
        if not scraper.verificar_sesion():
            ...
            if usuario and password:
                logger.info("Sesión expirada. Repitiendo login automático...")
                # Forzar recreación con login
                os.environ["SCREENERMATIC_PHPSESSID"] = ""
                scraper = ScreenermaticScraper.desde_env()
                if not scraper.verificar_sesion():
                    return self._fallback_viejo()
            else:
                return self._fallback_viejo()
    ...
    def _fallback_viejo(self) -> pd.DataFrame:
        df_viejo = self._cargar_desde_db()
        if not df_viejo.empty:
            logger.warning(
                "Sesión Screenermatic expirada y sin login. Sirviendo datos del caché "
