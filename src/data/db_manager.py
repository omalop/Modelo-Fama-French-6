...
class DBManager:
    ...
    def _get_outdated_tickers(self, tickers, category='prices'):
        ...
        for t in tickers:
            if t not in existing_dict:
                outdated.append(t)
            elif pd.isna(existing_dict[t]) or existing_dict[t] is None:
                outdated.append(t)
            else:
        ...
        if len(outdated) == 0 and len(tickers) > 0:
            logger.debug(f"DEBUG: Ticker {tickers[0]} existing_val={existing_dict.get(tickers[0])}")
    ...
    def _fetch_yfinance_fundamentals(self, ticker, session=None):
    ...
    def _get_session(self):
        """Crea una sesión de requests con headers realistas para evitar ser bloqueado."""
        session = requests.Session()
        user_agents = [
            'Omar Lopez (omlop90@gmail.com)',
    ...
    def update_history(self, tickers: list, source: str = 'yfinance'):
        ...
        logger.info(f"Outdated: Prices={len(tickers_prices)}, Financials={len(tickers_fin)}")
        ...
        try:
            ...
            if tickers_prices:
                ...
                # self._delete_data_for_tickers(tickers_prices, 'prices') # No borrar antes, mejor Upsert o borrar batch a batch
                
                batch_size = 1 # Ultra-lento pero seguro para evitar 429
                for i in range(0, len(tickers_prices), batch_size):
                    batch = tickers_prices[i:i + batch_size]
                    logger.info(f"Descargando batch {i//batch_size + 1}/{(len(tickers_prices)-1)//batch_size + 1}...")
                    
                    self._delete_data_for_tickers(batch, 'prices') # Borrar solo lo que vamos a descargar
                    
                    retry_count = 0
                    max_retries = 2
                    while retry_count <= max_retries:
                        try:
                            df_batch = yf.download(batch, period="10y", group_by='ticker', 
                            ...
                            if not df_batch.empty:
                                ...
                                logger.warning(f"Batch vacío. Reintentando...")
                                time.sleep(5)
                                retry_count += 1
                        except Exception as e:
                            if "Too Many Requests" in str(e) or "Rate limit" in str(e):
                                sleep_time = 30 * (retry_count + 1)
                                logger.warning(f"Rate limited. Durmiendo {sleep_time}s...")
                                time.sleep(sleep_time)
                                session = self._get_session() # Rotar headers
                                retry_count += 1
                            else:
                                logger.error(f"Error en yfinance para {batch}: {e}")
                                break
                    
                    # Pausa entre tickers para simular humano
                    time.sleep(2.5 + random.random() * 2)
                    
                    if i > 0 and (i // batch_size) % 10 == 0:
                        logger.info("Rotando sesión para evitar trackeo...")
                        session = self._get_session()
                    ...
                    elif 'Close' in df_long.columns:
                        df_long.rename(columns={'Close': 'close'}, inplace=True)
                        df_long = df_long.dropna(subset=['close'])
                    
                    # ALINEAR el orden exacto de las columnas de DuckDB
                    column_order = ['ticker', 'date', 'open', 'high', 'low', 'close', 'volume']
                    # Asegurar minusculas para match con DuckDB
                    df_long.columns = [c.lower() for c in df_long.columns]
                    
                    df_to_save = df_long[[c for c in column_order if c in df_long.columns]].copy()

                    # Inserción Directa a DuckDB
                    if not df_to_save.empty:
                        self.conn.execute("INSERT OR IGNORE INTO prices SELECT * FROM df_to_save")
                    ...
                    # Pequeño delay entre batches para ser gentiles
                    time.sleep(2)
                ...
                if source == 'sec':
                    ...
                    for i, ticker in enumerate(tickers_fin):
                        ...
                        try:
                            ...
                            yf_fund = self._fetch_yfinance_fundamentals(ticker, session=session)
                            if yf_fund: financials_data.extend(yf_fund)
                        except:
                            metadata_records.append((ticker, 'Unknown', 0, 'USD', datetime.now(), None, datetime.now()))
                        time.sleep(1.5)
