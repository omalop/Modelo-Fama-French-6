...
class FamaFrenchCalculator:
    ...
    def fetch_data(self):
        ...
        try:
            db = DBManager()
            logger.info(f"Tickers a actualizar: {self.tickers[:5]}... (Total: {len(self.tickers)})")
        ...
        # --- 2.5 Dolarización Sincrónica (Script Usuario) y Benchmarks ---
        session = requests.Session()
        session.headers.update({'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36'})

        def download_series(tkr, period='5y'):
            try:
                data = yf.download(tkr, period=period, auto_adjust=True, progress=False, session=session)
                if data.empty: return pd.Series(dtype=float)
                if isinstance(data.columns, pd.MultiIndex):
                    if 'Close' in data.columns: return data['Close'][tkr].copy()
                    elif 'Adj Close' in data.columns: return data['Adj Close'][tkr].copy()
                else:
                    if 'Close' in data.columns: return data['Close'].copy()
                    elif 'Adj Close' in data.columns: return data['Adj Close'].copy()
                return pd.Series(dtype=float)
            except Exception as e:
                logger.debug(f"Error descargando {tkr}: {e}")
                return pd.Series(dtype=float)

        ccl_series = None
        benchmark_returns = None

        logger.info("Descargando Benchmarks y/o Dólar CCL (Lógica de alineación operativa)...")
        try:
            if self.mode == 'argentina':
                # Dolarizacion CCL
                ggal_ba = download_series('GGAL.BA')
                time.sleep(2)
                ggal_adr = download_series('GGAL')
                time.sleep(2)
                
                if not ggal_ba.empty and not ggal_adr.empty:
                    ccl_df = pd.concat([ggal_ba, ggal_adr], axis=1, keys=['BA', 'ADR']).dropna()
                    ccl_series = (ccl_df['BA'] * 10) / ccl_df['ADR']
                    logger.info(f"Dólar CCL calculado exitosamente: {len(ccl_series)} días operativos conjuntos.")
                    
                bm_series = download_series('^MERV')
                if not bm_series.empty and ccl_series is not None:
                    bm_aligned = pd.concat([bm_series, ccl_series], axis=1, keys=['BM', 'CCL']).dropna()
                    bm_dolarized = bm_aligned['BM'] / bm_aligned['CCL']
                    benchmark_returns = np.log(bm_dolarized / bm_dolarized.shift(1)).dropna()
                    logger.info("Benchmark Local (^MERV) dolarizado al CCL.")
            else:
                bm_series = download_series('^SPX')
                if not bm_series.empty:
                    benchmark_returns = np.log(bm_series / bm_series.shift(1)).dropna()
                    logger.info("Benchmark Global (^SPX) descargado correctamente.")
                    
            if benchmark_returns is not None:
                benchmark_returns.index = pd.to_datetime(benchmark_returns.index).tz_localize(None)
                
        except Exception as e:
            logger.warning(f"Error descargando benchmarks o CCL: {e}")
            # Fallback a retornos neutros para no romper la matriz de covarianza
            benchmark_returns = pd.Series([0.0]*100, index=pd.date_range(end=datetime.now(), periods=100), name='BM')
        ...
        for i, ticker in enumerate(self.tickers):
            ...
            try:
                ...
                # Alinear fechas a Tz-Naive
                hist.index = pd.to_datetime(hist.index).tz_localize(None)

                # Dolarizacion CCL si es pertinente
                if self.mode == 'argentina' and ccl_series is not None:
                    ccl_naive = ccl_series.copy()
                    ccl_naive.index = pd.to_datetime(ccl_series.index).tz_localize(None)
                    
                    hist = hist.join(ccl_naive.rename('ccl'), how='inner') # Inner join alinea los dias habiles
                    for col_p in ['Open', 'High', 'Low', 'Close']:
                        hist[col_p] = hist[col_p] / hist['ccl']
                    hist.drop(columns=['ccl'], inplace=True)

                if not hist.empty:
                    price = hist['Close'].iloc[-1]
                else:
                    # FALLBACK: Intentar obtener precio actual via info (menos rate-limited)
                    try:
                        t_info = yf.Ticker(ticker).info
                        price = t_info.get('currentPrice', t_info.get('previousClose', 0))
                        logger.info(f"Fallback Price para {ticker}: {price}")
                    except:
                        price = 0
                
                mkt_cap = price * shares if (shares > 0 and price > 0) else price
                if sector == 'Unknown': sector = 'Financial' # Default neutral
                
                if price <= 0:
                    logger.warning(f"{ticker}: Precio <= 0. Omitiendo.")
                    continue 

                # --- CÁLCULO Mkt-RF (BETA) ---
                beta = np.nan
                logger.info(f"{ticker}: Analizando fundamentales...")
                if benchmark_returns is not None and len(hist) > 30:
                    hist_rets = np.log(hist['Close'] / hist['Close'].shift(1)).replace([np.inf, -np.inf], np.nan).dropna()
                    aligned_data = pd.concat([hist_rets, benchmark_returns], axis=1).dropna()
                    if len(aligned_data) > 30:
                        cov_matrix = np.cov(aligned_data.iloc[:, 0], aligned_data.iloc[:, 1])
                        var_market = np.var(aligned_data.iloc[:, 1])
                        if var_market > 0:
                ...
                if book_value is None:
                    logger.warning(f"{ticker}: No se encontró Book Value.")
                    continue
                ...
                if 'Total Assets' in bs.index:
                    ...
                    if len(assets) >= 2:
                        ...
                        logger.warning(f"{ticker}: Solo 1 reporte de Assets disponible.")
    ...
    def calculate_scores(self):
        ...
        # Limpieza básica
        cols_ff = ['Book_to_Market', 'Profitability', 'Asset_Growth', 'Beta', 'MarketCap']
        df[cols_ff] = df[cols_ff].apply(pd.to_numeric, errors='coerce')
        # Drop only if the core 3 aren't there. Beta and Size will be imputed if missing (e.g. 1.0 median)
        df.dropna(subset=['Book_to_Market', 'Profitability', 'Asset_Growth'], inplace=True)
        
        # Generar Log_Size para SMB
        df['Log_Size'] = np.log(df['MarketCap'].replace(0, np.nan))

        # --- 1. NORMALIZACIÓN ROBUSTA (Mediana/IQR) ---
        # Evita que outliers distorsionen el score
        def robust_zscore(x):
            median = x.median()
        ...
        cols_score = ['Book_to_Market', 'Profitability', 'Asset_Growth', 'Log_Size', 'Beta']
        for col in cols_score:
            z_col_name = f'Z_{col}'
            # Transformación robusta por sector (o universo completo)
            df[z_col_name] = df.groupby('Sector')[col].transform(robust_zscore)
            # Rellenar NaNs con 0 (mediana sectorial/global) para que componentes faltantes no castiguen ni premien
        ...
        # Penalización por TF menores si TF macro es bajista
        # Si la tendencia Trimestral/Mensual está rota (<3), el rebote de corto plazo no es confiable.
        mask_macro_bearish = (df['Mom_Status_3M'] < 3) & (df['Mom_Status_1M'] < 3)
        mask_short_bullish = (df['Mom_Status_1W'] >= 3) | (df['Mom_Status_1D'] >= 3)
        df.loc[mask_macro_bearish & mask_short_bullish, 'Raw_Mom_Score'] -= 4.0
        ...
        # Aplicar a TODAS las columnas Z para que el score sea consistente limitando distorsión.
        all_z_cols = [
            'Z_Value', 'Z_Prof', 'Z_Inv', 'Z_Size', 'Z_Beta', 'Z_Mom',
            'Z_Book_to_Market', 'Z_Profitability', 'Z_Asset_Growth', 'Z_Log_Size'
        ]
        ...
        # --- 4. SCORE FINAL (FAMA-FRENCH 6 MULTI-FACTOR + MOMENTUM) ---
        # Pesos Acordados: 40% Momentum, 60% Fama-French 5
        # Composición interna del 60% FF5: Value(15%), Prof(15%), Inv(10%), Size(10%), Beta(10%)
        # Es decir, coeficientes sobre el total (1.00):
        w_val  = 0.15 # 15% Book to Market (Premio Value)
        w_prof = 0.15 # 15% Quality/Profitability (Premio Rentabilidad)
        w_inv  = 0.10 # 10% Investment Growth (-Z)
        w_size = 0.10 # 10% Size/MarketCap (-Z, premiando Small Caps)
        w_beta = 0.10 # 10% Mkt-RF (-Z, baja volatilidad / Betting against Beta defect)
        w_mom  = 0.40 # 40% Momentum Técnico (Domenec)
        
        # Score Base
        df['Final_Score'] = (
            (w_val  * df['Z_Value']) +
            (w_prof * df['Z_Prof']) +
            (w_inv  * (-1 * df['Z_Inv_Capped'])) + # Castigamos el hiper-crecimiento de activos
            (w_size * (-1 * df['Z_Size'])) +       # Premiamos menor capitalización frente al resto (SMB)
            (w_beta * (-1 * df['Z_Beta'])) +       # Premiamos baja volatilidad estructural / Low Vol risk premium
            (w_mom  * df['Z_Mom'])
        )
        
        # Aplicar Penalización Gatekeeper (RMW) Extra
        # Ojo, la patada extra para empresas quemadoras de caja:
...
def main():
    ...
    parser.add_argument('--universe', type=str, choices=['argentina', 'global_sec', 'global_intl', 'all'], default='all')
    args = parser.parse_args()

    # 1. Ranking SEC (EE.UU. - Fuente Oficial)
    if args.universe in ['global_sec', 'all']:
    ...
    if args.universe in ['global_intl', 'all']:
    ...
    if args.universe in ['argentina', 'all']:
